...
const API          = "http://54.80.175.211:8080";
const WS_URL       = "http://54.80.175.211:8080/ws/game";
...
function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      try {
        const res = await fetch(`${API}/api/player/me`, { headers: { Authorization: `Bearer ${user.token}` } });
        if (!res.ok) return;
...
function Tag({ label, color = "var(--accent)" }) {
  return <span style={{
    fontFamily: "var(--mono)", fontSize: 10, letterSpacing: "0.1em", color,
    border: `1px solid ${color}50`, padding: "2px 8px", borderRadius: 3,
    textTransform: "uppercase", background: `${color}10`,
  }}>{label}</span>;
}
...
function Nav({ page, setPage }) {
  ...
  const navLinks = [
    ...
    { lbl: "Multiplayer",  action: () => { setMobileOpen(false); setPage("multiplayer"); } },
    { lbl: "News",         action: () => { setMobileOpen(false); setPage("news"); } },
      ...
      <div style={{ display: "flex", gap: "1.25rem", alignItems: "center" }}>
        ...
        {user ? (
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <button onClick={() => { logout(); setPage("home"); }} style={{ background: "none", border: "none", color: "var(--muted)", fontSize: 12, fontFamily: "var(--mono)", cursor: "pointer" }}>exit</button>
          </div>
        ) : (
    ...
    <div style={{ padding: "140px 2rem 90px", textAlign: "center", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative" }}>
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap", animation: "fadeUp .6s .3s ease both", opacity: 0, animationFillMode: "forwards" }}>
          <Btn v="primary" style={{ padding: "13px 28px", fontSize: 16 }} onClick={() => setPage("multiplayer")}>
            🎮 Play Together
          </Btn>
          <Btn v="outline" style={{ padding: "13px 28px", fontSize: 16 }} onClick={() => document.getElementById("leaderboard")?.scrollIntoView({ behavior: "smooth" })}>
            🏆 Leaderboard
          </Btn>
          <Btn v="outline" style={{ padding: "13px 28px", fontSize: 16 }} onClick={() => setPage("news")}>
            📋 Updates
          </Btn>
        </div>
      </div>
  ...
  return (
    <section id="leaderboard" style={{ padding: "3rem 2rem", maxWidth: 960, margin: "0 auto" }}>
      ...
      {loading ? <Loader /> : error ? <ErrBox msg={error} /> : rows.length === 0
        ? <p style={{ color: "var(--muted)", fontFamily: "var(--mono)", fontSize: 14 }}>No records yet for this level.</p>
        : (
...
function ProfilePage({ setPage }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      fetch(`${API}/api/quests`, { headers: hdrs }).then(r => r.ok ? r.json() : null).then(q => { if (q) setQ(q); }).catch(() => {});
    }, POLL_INTERVAL);
    return () => clearInterval(id);
  }, [user?.token]);
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      if (Object.keys(body).length === 0) { setEditOk("Nothing to change."); setEditL(false); return; }
  ...
  const claimQuest = async (qid) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        updateUser({ coins: (user.coins || 0) + data.reward });
      }
    } catch {}
    setClaiming(null);
  };
  ...
  const tabs = ["overview", "quests", "edit profile"];
  ...
  return (
    <div style={{ maxWidth: 780, margin: "0 auto", padding: "90px 2rem 4rem" }}>
      <Card style={{ padding: "2rem", marginBottom: "1.5rem", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
        ...
        <Btn v="danger" style={{ fontSize: 13, padding: "8px 16px" }} onClick={() => { logout(); setPage("home"); }}>Logout</Btn>
      </Card>
...
/* ── WebSocket hook — auto-disconnect on logout ──────────────────────────── */
function useWebSocketPlayers(token, username) {
  ...
  const connect = useCallback((level) => {
    ...
    ws.onopen = () => {
      ...
      ws.send(JSON.stringify({ x: 0, y: 0, lv: level, fr: true, anim: "idle", name: unRef.current }));
    };
    ws.onmessage = (e) => {
      try { setPlayers(JSON.parse(e.data).players || []); } catch {}
    };
    ws.onclose = () => { wsRef.current = null; setConnected(false); setMyLevel(null); setPlayers([]); };
    ws.onerror = () => { setConnected(false); };
  }, [token, _close]);

  // Disconnect immediately when token disappears (logout)
  useEffect(() => {
    if (!token) _close();
  }, [token, _close]);

  // Heartbeat ping every 2 s
  useEffect(() => {
    const id = setInterval(() => {
      if (wsRef.current?.readyState === WebSocket.OPEN)
        wsRef.current.send(JSON.stringify({ x: 0, y: 0, lv: lvRef.current, fr: true, anim: "idle", name: unRef.current }));
    }, 2000);
    return () => { clearInterval(id); _close(); };
  }, [_close]);

  return { players, connected, myLevel, connect, disconnect: _close };
}

/* ── Player avatar dot ───────────────────────────────────────────────────── */
function PlayerRow({ name, anim, isSelf, color }) {
  return (
    <div style={{
...
function MultiplayerPage({ setPage }) {
  ...
  const { players, connected, myLevel, connect, disconnect } = useWebSocketPlayers(user?.token, user?.username);
  const [tab, setTab] = useState("lobbies");

  // When user disconnects, go back to lobbies tab
    ...
    <div style={{ maxWidth: 960, margin: "0 auto", padding: "90px 2rem 4rem" }}>
      ...
      <div style={{ marginBottom: "2rem", display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 12 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{ fontFamily: "var(--mono)", fontSize: 12, color: connected ? "var(--accent)" : "var(--muted)" }}>
            {connected ? `Connected · Level ${myLevel} — ${LEVEL_NAMES[myLevel]}` : "Not in a lobby"}
          </span>
          {connected && (
            <Btn v="danger" style={{ fontSize: 12, padding: "5px 12px" }} onClick={disconnect}>Leave</Btn>
          )}
        </div>
      </div>
      ...
      {tab === "lobbies" && (
        <div>
                ...
                <Card key={l} style={{
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 }}>
                      <span style={{ fontFamily: "var(--mono)", fontSize: 10, color: diffColor,
                        background: `${diffColor}12`, border: `1px solid ${diffColor}40`,
                        padding: "2px 8px", borderRadius: 3, textTransform: "uppercase" }}>{diffLabel}</span>
                      <span style={{ fontFamily: "var(--mono)", fontSize: 11,
                        color: online > 0 ? "var(--accent)" : "var(--muted)" }}>
                  ...
                  {inThis ? (
                    <div style={{ display: "flex", gap: 8 }}>
                      <Btn v="outline" style={{ flex: 1, justifyContent: "center", fontSize: 13, padding: "8px 0" }} onClick={() => setTab("session")}>
                        View Session ↗
                      </Btn>
                      <Btn v="danger" style={{ fontSize: 13, padding: "8px 12px" }} onClick={disconnect}>Leave</Btn>
                    </div>
                  ) : (
                    <Btn style={{ width: "100%", justifyContent: "center", fontSize: 13 }}
                      onClick={() => joinLobby(l)}>
          ...
          {/* How it works */}
          <Card style={{ padding: "1.5rem", background: "var(--bg1)" }}>
        ...
        <div>
          <Card style={{ padding: "1.5rem", marginBottom: "1.5rem" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              {players.length === 0 ? (
                <div style={{ padding: "10px 14px", background: "var(--bg2)", borderRadius: 6,
                ...
                <PlayerRow key={p.id ?? i} name={p.username || `Player #${p.id}`} anim={p.anim} color="rgba(100,180,255,0.85)" />
              ))}
            </div>
          </Card>
...
function MiniLeaderboard({ level }) {
  ...
  if (rows.length === 0) return <p style={{ color: "var(--muted)", fontSize: 13 }}>No records yet.</p>;
